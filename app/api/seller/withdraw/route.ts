...
export async function POST(req: NextRequest) {
  // FIX: requireAuth() was called before the try block below, so if it threw
  // (e.g. a transient Supabase/D1 error inside getRoleByUid) the exception
  // propagated straight out of this handler with no JSON body at all — the
  // client's res.json() then failed with "Unexpected end of JSON input",
  // hiding what actually went wrong. Wrapping this call means every failure
  // path from here on returns a real, parseable error response.
  let auth
  try {
    auth = await requireAuth(req)
  } catch (err: any) {
    console.error("[POST /api/seller/withdraw] auth check failed:", err)
