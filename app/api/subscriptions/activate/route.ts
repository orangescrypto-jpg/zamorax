...
// Called from the seller's subscription success redirect for Paystack-paid
// plan upgrades. Verifies the transaction directly with Paystack (never
// trusts the client-supplied "it succeeded"), then activates the plan the
// same way /api/payment/confirm does for admin-confirmed manual payments.
// Manual bank-transfer subscriptions are NOT touched here — those stay
// "pending_payment" until an admin confirms them at /admin/payments, exactly
// like today's boost/adBoost flow.
...
export async function POST(req: NextRequest, context: RouteContext) {
  ...
  try {
    ...
    await d1Query(
      ...
      ["active", reference, "paystack", now, subscriptionId],
      nativeDB,
    )
    await d1Query(
