...
export default function NewDisputePage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Send dispute emails to both buyer and seller (fire-and-forget)
      const itemTitle = (order.itemTitle as string) || `Order #${orderId.slice(-6).toUpperCase()}`
      const buyerEmail  = user.email  as string | undefined
      const sellerEmail = order.sellerEmail as string | undefined
      if (buyerEmail) {
        Emails.disputeOpened(buyerEmail, {
          recipientName: user.fullName || "Buyer",
          role:          "buyer",
          itemTitle,
          orderId,
          reason,
      ...
      if (sellerEmail) {
        Emails.disputeOpened(sellerEmail, {
          recipientName: (order.sellerName as string) || "Seller",
          role:          "seller",
          itemTitle,
          orderId,
          reason,
  ...
  if (!settings.disputeFilingEnabled) {
    return (
      <div className="container max-w-lg py-12 text-center space-y-4 pb-24">
        <div className="h-14 w-14 rounded-full bg-muted flex items-center justify-center mx-auto">
          <ShieldAlert className="h-7 w-7 text-muted-foreground" />
        </div>
        <h2 className="text-xl font-heading font-bold">Dispute Filing Unavailable</h2>
        <p className="text-sm text-muted-foreground">
          Dispute filing is temporarily unavailable. Please try again later or contact support.
        </p>
      </div>
    )
  }
