...
export default function ChatListPage() {
  ...
  useEffect(() => {
    ...
    return onSnapshot(q, docs => {
      setChats(docs.map(d => ({ id: d.id, ...d.data() })))
      setLoading(false)
    }, () => setLoading(false))
  }, [user?.uid])
    ...
    <main className="container max-w-lg py-6 pb-24 space-y-4">
      ...
      {chats.length === 0 && (
        <div className="flex flex-col items-center justify-center py-20 text-center gap-3">
          ...
          <p className="text-sm text-muted-foreground">When you contact a seller, your chats will appear here.</p>
        </div>
      )}

      {chats.map(chat => {
        const otherId = chat.participants?.find((p: string) => p !== user?.uid)
        const otherName = chat.participantNames?.[otherId] || "Seller"
        const lastMsg = chat.lastMessage || "No messages yet"
        const time = chat.lastMessageAt?.toDate ? formatDistanceToNow(chat.lastMessageAt.toDate(), { addSuffix: true }) : ""
