...
export default function HubVerifyPage() {
  ...
  useEffect(() => {
    ...
    unsubs.push(AdminService.subscribeToCollection("listings", docs => { setListings(docs.docs.map(d => ({ id: d.id, ...d.data() }))); setLoading(false) }, [where("status", "==", "active")]))
    unsubs.push(AdminService.subscribeToCollection("hubVerificationRequests", docs => setHubRequests(docs.docs.map(d => ({ ...d }, [where("sellerId", "==", uid)])))
    ))
