...
export default function BlogIndexClient({ initialPosts = [] }: { initialPosts?: BlogPost[] }) {
  // initialPosts come from the server so the article links/titles are in the HTML.
  ...
  useEffect(() => {
    if (initialPosts.length > 0) return // server already provided them
