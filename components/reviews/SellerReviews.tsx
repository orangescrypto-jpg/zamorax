...
export function WriteReviewModal({ sellerId, orderId, sellerName, open, onClose }: {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Deterministic doc ID prevents duplicate reviews for the same order.
      // Matches the Firestore rule: reviewId == orderId + "_" + buyerId
      const reviewDocId = `${orderId}_${user.uid}`
      const existing = await AdminService.getDoc("reviews", reviewDocId)
      if (existing) {
        toast({ title: "Already reviewed", description: "You have already left a review for this order.", variant: "destructive" })
        onClose()
        return
      }

      await AdminService.setDoc("reviews", reviewDocId, {
