...
export function ReviewSystem({ sellerId, orderId, showForm }: ReviewSystemProps) {
  ...
  useEffect(() => {
    const unsub = AdminService.subscribeToCollection(
      "reviews",
      docs => { setReviews(docs.map((d: any) => ({ ...d } as Review))); setLoading(false) },
  ...
  const handleSubmit = async () => {
    ...
    setSubmitting(true)
    try {
      // Use a deterministic doc ID: "{orderId}_{buyerId}"
      // Matches the Firestore rule — prevents duplicate reviews for the same order.
