"use client"
// components/shared/WatermarkedImage.tsx
// imageWatermarkEnabled from config/platform controls the overlay.
// The prop watermark={false} still works as a local override (e.g. admin preview).
...
interface WatermarkedImageProps {
  ...
  height?: number
  className?: string
  watermark?: boolean   // explicit prop overrides platform setting if false
  sizes?: string
  priority?: boolean
}

export function WatermarkedImage({
  src, alt, fill, width, height, className, watermark, sizes, priority
}: WatermarkedImageProps) {
  const { settings } = usePlatformSettings()
  // Show watermark if: admin enabled it AND caller hasn't explicitly disabled it
  const showWatermark = watermark !== false && settings.imageWatermarkEnabled
