...
// Reads chatbotEnabled + chatbotGreeting from config/platform via usePlatformSettings.
// Returns null instantly when disabled — no flash.
...
export default function ChatbotWidget({ listingTitle, listingId }: ChatbotWidgetProps) {
  ...
  const greeting = settings.chatbotGreeting ||
    (listingTitle
  ...
  // Update greeting if it changes from settings (first render uses default)
  useEffect(() => {
    setMessages([{ role: "assistant", content: greeting }])
  }, [settings.chatbotGreeting]) // eslint-disable-line react-hooks/exhaustive-deps
  ...
  // ── Gate: admin disabled chatbot ──────────────────────────────────────────
  if (!loading && !settings.chatbotEnabled) return null
          ...
          <div className="p-3 border-t border-border shrink-0">
            <div className="flex gap-2">
              <Input
                ...
                disabled={isTyping}
              />
              <Button size="icon" onClick={handleSend} disabled={!input.trim() || isTyping} className="h-9 w-9 shrink-0">
                {isTyping ? <Loader2 className="w-4 h-4 animate-spin" /> : <Send className="w-4 h-4" />}
              </Button>
            </div>
            <p className="text-[10px] text-muted-foreground/50 text-center mt-2">Powered by Zamorax AI · Claude</p>
          </div>
        </div>
      )}
    </>
