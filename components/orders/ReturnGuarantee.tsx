...
// ─────────────────────────────────────────────
// 1. RETURN GUARANTEE BADGE
// Drop this anywhere on listing page near price/trust badges
// ─────────────────────────────────────────────
  ...
  return (
    <div className="bg-blue-50 border border-blue-100 rounded-xl p-4 space-y-2">
      <div className="flex items-center gap-2">
        ...
        <div>
          <p className="text-sm font-semibold text-blue-800">3-Day Return Guarantee</p>
          <p className="text-xs text-blue-600">Changed your mind? Return it within 3 days.</p>
        </div>
      </div>
...
// Drop in buyer order detail page: /dashboard/buyer/orders/[id]/page.tsx
// Show only if order status === "completed" AND within 3 days of delivery
// ─────────────────────────────────────────────
interface ReturnRequestFormProps {
  ...
  listingTitle: string
  sellerId: string
  deliveredAt: Date // pass order's deliveredAt timestamp
}
...
export function ReturnRequestForm({
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Also create an in-app notification for admin
      ...
      toast({
        ...
        description: "Our team will review within 24 hours. Escrow funds are held.",
        variant: "success",
      })
      setOpen(false)
    } catch (e) {
  ...
  if (!withinWindow) {
    return (
      <div className="text-xs text-muted-foreground flex items-center gap-1.5 bg-muted/50 px-3 py-2 rounded-lg">
        ...
        Return window has closed ({RETURN_WINDOW_DAYS}-day policy)
      </div>
    )
  }
